...
class RedmineResources::Charts::AllocationChart < Redmine::Helpers::Gantt
  ...
  def initialize(project, query, options = {})
    ...
    @holidays = PeopleHoliday.where(is_workday: false).between(date_from, date_to) if RedmineResources.people_pro_plugin_installed?
  end
  ...
  def render_user(user, options = {})
    ...
    options[:user_line_map] = user_line_map_hash(user, resource_bookings_by(user))
    ...
    when 3
      wrap(:lines, :div, class: 'user-line') do
        ...
        while i < user_line_map.length
          ...
          if day_plan && day_plan[:scheduled_hours] > 0
    ...
    if start_date && end_date && start_date <= self.date_to && end_date >= self.date_from
      ...
      when 1 # month scale
        ...
        if end_date <= self.date_to
          months_from_start = (end_date.end_of_month.year * 12 + end_date.end_of_month.month) - (self.date_from.year * 12 + self.date_from.month)
          coords[:end] = coords[:bar_end] = (months_from_start + 1)
        else
          coords[:bar_end] = (self.date_to.year * 12 + self.date_to.month - self.date_from.year * 12 + self.date_from.month)
        end
      when 2 # week scale
        if start_date >= self.date_from
        ...
        if end_date <= self.date_to
          ...
          coords[:bar_end] = self.date_to - self.date_from
        end
      else # day scale
        if start_date >= self.date_from
  ...
  def user_line_map_hash(user, resource_bookings)
    if RedmineResources.people_pro_plugin_installed?
      ...
      person_capacity_hash = Dayoff.person_available_capacity_by_day(person, @date_from, @date_to)
      person_available_capacity_by_day_with_holidays(person_capacity_hash)
    else
  ...
  def person_available_capacity_by_day_with_holidays(person_capacity_hash)
    person_capacity_hash.each do |date, capacity|
