...
class TestBacktrackingFasterThanILP(unittest.TestCase):
    ...
    def compute_total_rating(self, result, issues, users):
        """Compute the total rating of the assigned users."""
        total = 0
        for issue_id, roles in result.items():
            issue = next(i for i in issues if i.id == issue_id)
            for role, user_ids in roles.items():
                for user_id in user_ids:
                    user = next(u for u in users if u.id == user_id)
                    total += get_rating(user, role, issue.category)
        return total

    def count_unique_users(self, result):
        """Count the number of unique users in the solution."""
        return len(set(user_id for issue_id, roles in result.items() for role, user_ids in roles.items() for user_id in user_ids))

    def test_backtracking_faster_than_ilp(self):
        """Test that backtracking is faster than ILP and produces an optimal solution."""
        ...
        # Define 10 users with varied ratings
        users = [
            User({
                "id": i + 1,
                ...
                "lastname": "Test",
                "qualifications": [
                    {"role": "Senior Engineer", "category": "Network", "rating": 8 + (i % 3)},  # Ratings 8, 9, 10
                    {"role": "Network Specialist", "category": "Network", "rating": 8 + (i % 3)}
                ],
        ...
        def backtrack_wrapper(issue_idx):
            recursion_count[0] += 1
            print(f"Wrapper called for issue_idx {issue_idx}")  # Debug
        ...
        # Verify optimality: total rating
        backtrack_total_rating = self.compute_total_rating(backtrack_result, issues, users)
        ilp_total_rating = self.compute_total_rating(ilp_result, issues, users)
        self.assertGreaterEqual(backtrack_total_rating, ilp_total_rating,
                               f"Backtracking rating {backtrack_total_rating} should be >= ILP rating {ilp_total_rating}")

        # Verify unique users (optional, for workload distribution)
        backtrack_unique_users = self.count_unique_users(backtrack_result)
        ilp_unique_users = self.count_unique_users(ilp_result)
        print(f"Backtracking unique users: {backtrack_unique_users}, ILP unique users: {ilp_unique_users}")
        ...
        print(f"ILP total rating: {ilp_total_rating}")
        print(f"Backtracking total rating: {backtrack_total_rating}")
