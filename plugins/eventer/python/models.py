...
class CustomFilter:
    def __init__(self, name: str, conditions: dict):
        self.name = name
        self.conditions = conditions  # Raw JSON for placeholders
...
class User:
    def __init__(self, user_data: Dict):
        ...
        self.custom_filters = [
            CustomFilter(**cf) for cf in user_data.get("custom_filters", [])
        ]
    ...
    def is_available(self, start: datetime, end: datetime) -> bool:
        ...
        We'll do that logic in 'process.py'.
        """
...
class Issue:
    def __init__(self, issue_data: Dict):
        ...
        start_str = issue_data.get("start_datetime")
        self.start_datetime = parse_datetime(issue_data.get("start_datetime"))
        self.end_datetime = parse_datetime(issue_data.get("end_datetime"))

        self.category = issue_data["category"]
        self.category_priority = issue_data["category_priority"]
        self.priority = issue_data["priority"]
        # Build required roles
        self.required_roles = [
